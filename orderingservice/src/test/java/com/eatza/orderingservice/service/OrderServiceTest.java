...
import com.eatza.order.proxy.OrderToRestaurantProxy;
...
@ContextConfiguration(classes = { CustomGlobalExceptionHandler.class })
@SpringBootTest
public class OrderServiceTest {
	...
	@Mock
	private OrderToRestaurantProxy orderToRestaurantProxy;
	...
	public void placeOrder_basic() throws OrderException {
		...
		when(orderToRestaurantProxy.getMenuItemById(anyLong())).thenReturn(item);
	...
	public void placeOrder_no_item() throws OrderException {
		...
		when(orderToRestaurantProxy.getMenuItemById(anyLong())).thenReturn(null);
		when(itemService.saveItem(any(OrderedItem.class))).thenReturn(new OrderedItem());
	...
	public void updateOrder_quantityLess() throws OrderException {
		...
		MenuFetchDto menu = new MenuFetchDto(1L, "10", "22", restaurant);
		ItemFetchDto item = new ItemFetchDto(1L, "Onion Dosa", "Dosa", 110, menu);
		// OrderUpdateDto orderdto= new OrderUpdateDto(1,1,new
		// List<OrderedItemsDto>(1,10),1);
		when(orderToRestaurantProxy.getMenuItemById(anyLong())).thenReturn(item);
		// orderService.updateOrder(new OrderUpdateDto(1L, 1L, Arrays.asList(new
		// OrderedItemsDto(1L, 0)), 1L));
	...
	public void updateOrder_basic() throws OrderException {
		...
		// .updateOrder(new OrderUpdateDto(1L, 1L, Arrays.asList(new OrderedItemsDto(1L,
		// 1)), 1L));
